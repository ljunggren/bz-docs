...
do
  ...
  nohup docker run --rm --shm-size=1g -v "$(pwd):/var/boozang/" --name=bzworker${counter} styrman/boozang-playwright "${WORKER_URL}" > out_${counter}.log &
done

echo All workers done. Starting master job.
...
docker run --rm --shm-size=1g -v "$(pwd):/var/boozang/" --name=bzworker1 styrman/boozang-playwright "${MASTER_URL}"
```
